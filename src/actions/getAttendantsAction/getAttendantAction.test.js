...
import axios from '../../helpers/axiosHelper/getAttendantsRequest';
...
describe('actions for get attendant', () => {
  ...
  it('should trigger an initial loading action when called', () => {
    actions.storeAttendants()(dispatch);
    expect(dispatch).toBeCalled();
    expect(dispatch).toBeCalledWith({ type: GET_ATTENDANTS_LOADING });
    store.clearActions();
  });

  it('should dispatch success action type and response as payload', async () => {
    axios.getAttendantsRequest = jest.fn().mockResolvedValue(response);
    await actions.storeAttendants()(dispatch);
  ...
  it('should dispatch failure action type and response as payload', () => {
    jest.mock('axios');
    axios.getAttendantsRequest.mockImplementation(() => {
      throw new Error();
    });
    const dispatchMock = jest.fn();
    actions.storeAttendants()(dispatchMock);
  });
});
