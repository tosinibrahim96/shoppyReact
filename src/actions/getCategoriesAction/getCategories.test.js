...
import axios from '../../helpers/axiosHelper/getCategoriesRequest';

const { GET_CATEGORIES_SUCCESS, GET_CATEGORIES_LOADING } = actionTypes;
...
describe('actions for get attendant', () => {
  ...
  it('should trigger an initial loading action when called', () => {
    actions.productCategories()(dispatch);
    expect(dispatch).toBeCalled();
    expect(dispatch).toBeCalledWith({ type: GET_CATEGORIES_LOADING });
    store.clearActions();
  });

  it('should dispatch success action type and response as payload', async () => {
    axios.getCategoriesRequest = jest.fn().mockResolvedValue(response);
    await actions.productCategories()(dispatch);
  ...
  it('should dispatch failure action type and response as payload', () => {
    jest.mock('axios');
    axios.getCategoriesRequest.mockImplementation(() => {
      throw new Error();
    });
    const dispatchMock = jest.fn();
    actions.productCategories()(dispatchMock);
  });
});
